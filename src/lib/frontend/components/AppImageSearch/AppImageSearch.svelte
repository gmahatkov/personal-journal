<script lang="ts">
	...
	$: pagesInView = Object.entries(pages).reduce(
		(acc, [token, images], idx, arr) => {
	...
	async function loadImages() {
		...
		try {
			const res = await fetch(
				'/api/search/google-drive?' + (nextPageToken ? `&nextPageToken=${nextPageToken}` : '')
			);
...
{#each pagesInView as [token, page] (token)}
	<div
		class="grid grid-cols-4 gap-6 mb-6"
		data-token={token}
		use:trapIntersection={{ cb: onScroll }}
	>
		{#each page as item (item.id)}
			<AppImageSearchItem
				{item}
				{loading}
				on:imageLoaded={(evt) => setItemCache(item, evt.detail)}
			/>
		{/each}
	</div>
{/each}
