	...
	async function loadImage(observer: IntersectionObserver) {
		...
		try {
			const res = await fetch(
				item.thumbnailUrl ? `${item.url}?thumbnail=${item.thumbnailUrl}` : item.url
			);
...
<Card
	class="items-center max-w-[18rem] {showCard ? 'cursor-pointer' : 'cursor-progress'}"
	on:click={select}
>
	<div use:trapIntersection={{ cb: (_, __, observer) => loadImage(observer), threshold: 0.5 }} />
	{#if showCard && srcBase64.length}
		<Img src={srcBase64} alt={item.name} size="cover" caption={item.name} />
	{:else}
